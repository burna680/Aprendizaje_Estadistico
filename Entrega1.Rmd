...
```{r}
newdata = data.frame(varsPregnancies = 2, varsGlucose = 100, varsBloodPressure = 70, varsSkinThickness = 20, varsDiabetesPedigreeFunction = 0.24, varsAge = 30, varsInsuline = 0)
...
```{r}
IC <-predict(modelo, newdata = newdata, interval = "confidence") #Intervalo de confianza por default es .95
```
...
# ggplot(data=data, aes(x=modelo$residuals, y=modelo$fitted.values)) + 
#   geom_point(col="gray0", fill="darkorchid3", alpha = .9) + 
#   labs(title="Residuos", x="BMI", y="Valores") + xlim(c(0,length(modelo$fitted.values))) + ylim(c(min(modelo$fitted.values),max(modelo$fitted.values)))

ggplot(data=data, aes(x=modelo$residuals, y=modelo$fitted.values)) +    geom_point(col="gray0", fill="darkorchid3", alpha = .9) + 
   labs(title="Residuos", x="BMI", y="Valores") + xlim(c(0,length(modelo$fitted.values))) + ylim(c(min(modelo$fitted.values),max(modelo$fitted.values)))

qqnorm(modelo$residuals, col = "mediumpurple3", pch = 20, xlab = "Cuantiles teóricos", ylab = "Cuantiles estimados", main = "Comparación")
