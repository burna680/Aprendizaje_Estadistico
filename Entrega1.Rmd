...
```{r}
exhaustive = stepwise(clean_data)
summary(exhaustive)
```

```{r}
clean_data_insuline = adjust_data(clean_data, "Insulin", equals, 0)
exhaustive = stepwise(clean_data_insuline)
summary(exhaustive)
```
...
```{r}
newdata = data.frame(Pregnancies = 2, Glucose = 100, BloodPressure = 70, SkinThickness = 20, Outcome = as.factor(1), DiabetesPedigreeFunction = 0.24, Age = 30, Insuline = 0)
